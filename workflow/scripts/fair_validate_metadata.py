...
ACCEPTED_VERSION_EXCEPTIONS: dict[tuple[str, str], str] = {
    ("gbm_cellxgene_56c4912d_full/scvi_integration_provenance.json", "torch"): (
        ...
        "concordance) was rebuilt 2026-08-06 under the enforced env. "
        "RESEARCHER DECISION 2026-09-11: retain this artifact; do NOT re-derive. "
        "Re-deriving means a multi-hour scVI retrain on 1,006,344 cells that "
        "renumbers Leiden clusters and invalidates every per-cluster table keyed on "
        "nerve_c{N}. The capped arm's own integration recorded torch 2.12.0 and is "
        "clean, so the cross-arm replication evidence does not rest on this file. "
        "NOTE the pin itself stays at 2.12.0 deliberately: it is the environment "
        "every other artifact was built under, and downgrading it to match this one "
        "file would make 15 currently-conformant artifacts non-conformant. This "
        "exception therefore covers one historical artifact, not a go-forward "
        "environment choice — a future re-run of ds_scrna_integration will record "
        "2.12.0 and simply stop matching this entry. "
        "Closed in markdowns/task_conda_env_enforcement.md."
    ),
