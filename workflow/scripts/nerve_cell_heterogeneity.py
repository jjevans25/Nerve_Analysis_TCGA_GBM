...
if marker_rows:
    ...
    markers_df = pd.DataFrame(columns=["cluster", "names", "scores", "logfoldchanges", "pvals", "pvals_adj"])
...
try:
    ...
    for cluster in markers_df["cluster"].unique():
        ...
        for gs in gene_sets:
            try:
                ...
                log_transformation(log, "nerve_cell_heterogeneity",
                    f"WARNING: enrichr failed for cluster {cluster}, {gs}: {exc}", status="WARNING")
    ...
    log_transformation(log, "nerve_cell_heterogeneity",
        "WARNING: gseapy not installed — GSEA skipped", status="WARNING")

enrichment_df = pd.concat(enrichment_rows, ignore_index=True) if enrichment_rows else pd.DataFrame()
...
present_markers = [g for g in all_markers_flat if g in adata.var_names]

if len(present_markers) >= 1:
    fig, ax = plt.subplots(figsize=(max(8, len(present_markers) * 0.45), max(4, n_clusters * 0.6)))
    sc.pl.dotplot(
        adata_de,
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.text(0.5, 0.5, "No canonical markers present in dataset",
            ha="center", va="center", transform=ax.transAxes, fontsize=12)
    fig.savefig(snakemake.output.dotplot, dpi=100, bbox_inches="tight")
    plt.close(fig)
    log_transformation(log, "nerve_cell_heterogeneity",
        "WARNING: no canonical markers present — empty dot plot written", status="WARNING")
...
fig, ax = plt.subplots(figsize=(max(6, n_clusters * 0.8), max(5, abundance.shape[0] * 0.4)))
sns.heatmap(
    abundance,
    ax=ax,
    cmap="YlOrRd",
...
if "primary_diagnosis" in adata.obs.columns:
    diag_map = (
        adata.obs.groupby("sample_id")["primary_diagnosis"].first().to_dict()
    )
...
log_transformation(log, "nerve_cell_heterogeneity", "Complete", status="SUCCESS",
                   artifact_paths=[
                       snakemake.output.markers, snakemake.output.enrichment,
                       snakemake.output.dotplot, snakemake.output.abundance,
                   ])
